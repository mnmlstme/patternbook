...
function generateScopeInitial(scope) {
  return ['let initial = {'].concat(
    Object.keys(scope).map(variable => variable + ':' + JSON.stringify(scope[variable]) + ','),
    ['}']
  );
}

function generateStyles(styles) {
  return ['let styles = ['].concat(styles.map(file => `require('${file}'),`), ['].join("")']);
}

function generateSymbols(symbols) {
  let keys = Object.keys(symbols);
  return ['let symbols = {__html: ['].concat(
    keys.map(k => `Patternbook.convertSvgToSymbol('${k}',require('${symbols[k]}')),`),
    ['].join("")}']
  );
}
...
function toModule(payload) {
  ...
  let output = ["import React from 'react';", "import Patternbook from 'patternbook';"].concat(
